...
#define OUTPUT_TYPE 1          //0 for full output, 1 to print pi(x) only, 2 to print pi(x) components only
...
int main (int argc, char *argv[])
{
    ...
    uint64_t ttN = lpow (N, 2, 3);

    size_t psize = (1.26 * crN) / log (crN) * sizeof (uint64_t) + 1;
    uint64_t* primes = malloc (psize);
    assert (primes != NULL);
