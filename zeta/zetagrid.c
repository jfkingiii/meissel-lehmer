...
#define PROGRESS 0                                                 /*Report the progress of the calculation?*/
...
main(int argc, char *argv[])
{
  ...
  int decimals=10;
  /*  for(sigma=0; sigma<=1; sigma+=.1){
    for(t=5; t<=100; t+=.1){
      printf("%f\t\t%f\t\t", sigma, t);
      zeta(sigma, t, decimals, answer);
      mpc_abs(norm, answer, MPC_RNDNN);
      mpfr_out_str(stdout, 10, 10, norm, MPFR_RNDN);
      printf("\n"); 
    }
    }*/

  for (sigma=0; sigma<=1; sigma+=.01){
    zeta(sigma, 14.134725141734, decimals, answer);
    mpc_out_str(stdout, 10,10, answer, MPC_RNDNN);
    printf("\n");
  }
