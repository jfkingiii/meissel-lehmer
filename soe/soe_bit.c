...
int main(int argc, char *argv[])
{
  ...
  uint64_t n = atol(argv[1]) * pow(atol(argv[2]), atol(argv[3]));

  uint64_t* s = malloc((n/64+2)*sizeof(uint64_t));
  assert(s != NULL);
  uint64_t init = 0xAAAAAAAAAAAAAAAA;     //avoid need to sift even numbers by memsetting ..101010
  ...
  memset(s, init, sizeof(uint64_t) * (n/64+2));
  uint64_t sieve_to=floor(sqrt(n));
