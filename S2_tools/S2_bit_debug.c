...
uint64_t sieve(uint64_t N, uint64_t* p) //a bit ugly but exhaustively tested
{
  uint64_t M = N/2 + N%2;
  uint64_t i, j;
...
void dump_sieve(const uint8_t* s1, const uint64_t* s2, uint64_t l, uint64_t u,  uint64_t ans1, uint64_t ans2)
{
  printf("Dumping Sieve...\n");
  printf("Count from %lu to %lu = %lu\t%lu\n", l,u, ans1, ans2);
  uint64_t i;
  for (i=l; i<=u; i++) {printf("%d ",s1[i]); if ((i+1)%64==0) printf("\n");}
  printf("\n\n");
  for(i=l/64; i<=u/64+1; i++) printbitssimple(s2[i]);
  printf("\n");
}
...
int main(long argc, char *argv[])
{
  ...
  uint64_t* phi2 = malloc((pi_crn+1)*sizeof(uint64_t));
  memset(phi2, 0, (pi_crn+1)*sizeof(uint64_t));
  ...
  //bit sieve
  uint64_t* s2 = malloc((crn/64+2)*sizeof(uint64_t));
  assert(s2 != NULL);
  ...
  for(block_id = start_block; block_id <= finish_block; block_id++)
  {
    ...
    memset(s2, ~0, sizeof(uint64_t) * (crn/64+2));
	...
	if(mu[m] != 0 && p < lpf[m] && p*m > crn && m <= crn)
	{
	  ...
	  assert(q<=U);
	  cnt1=0, cnt2=0;
	  for(k=start; k<=q; k++) cnt1 += s[k-L];
	  phi[i] += cnt1;
	  cnt2 = count_bits(s2, start - L, q - L);
	  phi2[i] += cnt2;
	  if(cnt1!=cnt2) dump_sieve(s, s2, start-L,q-L,cnt1, cnt2 );
	  start = q+1;
      ...
      cnt1=0; cnt2=0;
      for(k = start; k < U; k++) cnt1 += s[k - L]; 
      phi[i]+=cnt1;
      cnt2 = count_bits(s2, start - L, U -1 - L);
      phi2[i]+=cnt2;
      if(cnt1!=cnt2) dump_sieve(s, s2, start-L,U-L-1, cnt1, cnt2);
  ...
  //  for(i=1; i<=pi_crn; i++) printf("phi[%lu] = %lu\n", i, phi[i]);
