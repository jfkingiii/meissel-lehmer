//THIS APPROACH WILL PROBABLY NOT WORK BECAUSE THE TOTAL S2 IS NOT THE SUM OF THE COMPUTATIONS OF THE
//THREADS -- NEED TO CUMULATE PHI AND MU - SEE THE S2_total.c file
...
#pragma omp parallel num_threads(NUM_THREADS) private(s, L, U, i, p, buffer1, buffer2, first_m, last_m, start, m, cum_mu, q, phi, S2)
  {
    if (omp_get_thread_num() == 0) printf("Running %d threads\n", omp_get_num_threads());

    s = malloc((crn/64+2)*sizeof(uint64_t));
    assert(s != NULL);

    phi = malloc((pi_crn+1)*sizeof(uint64_t));
    assert(phi != NULL);
    memset(phi, 0, (pi_crn+1)*sizeof(uint64_t));

    cum_mu = malloc((pi_crn+1)*sizeof(int64_t));
    assert(cum_mu != NULL);
...
#pragma omp for schedule(dynamic)
      ...
      for(i=1; i<=pi_crn; i++)
	{
	  ...
	  for(m = first_m; m > last_m && p*m > crn ; m--)
	      ...
	      if(mu[m] != 0 && p < lpf[m] && p*m > crn && m <= crn)
		{
		  ...
		  start = q+1;
		  if(mu[m] > 0) mpz_sub_ui(S2, S2, mu[m] * phi[i]);
		  else mpz_add_ui(S2, S2, -mu[m] * phi[i]);
		}
	    }
  ...
  #pragma omp critical
  {
    ...
    printf("\n");
    mpz_add(S2_total, S2_total, S2);
    free(s);  free(phi); free(cum_mu);
    mpz_clear(buffer1); mpz_clear(buffer2); mpz_clear(S2);
  }
  }                //end parallel block                
...
#endif
  mpz_out_str (stdout, 10, S2_total);
  printf("\n");
  mpz_clear(S2_total);
